...
OFFSET = CONFIG.get("offset", 0)  # Default offset for HX711 (zero-point reading)
SCALE = CONFIG.get("scale", 1.0)  # Default scale factor (N·cm per count)
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS torque_data (
            ...
            torque_value REAL  -- Changed to REAL to store calculated torque (float)
        )
    """)
    conn.commit()
    conn.close()
            ...
            async with client:
                ...
                while client.is_connected and not stop_ble:
                    try:
                        ...
                        # Ensure raw data is 4 bytes (32-bit) as expected
                        if len(raw) != 4:
                            status = f"Unexpected data length: {len(raw)} bytes (expected 4)"
                            await asyncio.sleep(1.0)
                            continue

                        # Convert to 32-bit integer
                        raw_val = int.from_bytes(raw, byteorder="little", signed=False)
                        print(f"Raw value (integer): {raw_val}")

                        # Log OFFSET and SCALE for debugging
                        ...
                        print(f"Read error details: {str(e)}")
                        await asyncio.sleep(1.0)
                        break
                    await asyncio.sleep(1.0)
...
def export_pdf():
    ...
    try:
        ...
        y = 780
        for ts, v in rows:
            c.drawString(100, y, f"{ts} – {v:.2f} N·cm")  # Format torque to 2 decimal places
            y -= 15
            if y < 50:
                c.showPage()
                y = 800
