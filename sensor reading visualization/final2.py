...
OFFSET = CONFIG.get("offset", 880804)  # Approx raw value for 2.1 mV (zero torque)
SCALE = CONFIG.get("scale", 8.39e-6)  # Placeholder: N·cm per count, adjust based on calibration
            ...
            async with client:
                ...
                def notification_handler(sender, data):
                    ...
                    try:
                        ...
                        # Ensure data is 3 bytes (24-bit) for HX711
                        if len(data) != 3:
                            status = f"Unexpected notification data length: {len(data)} bytes (expected 3)"
                            print(status)
                            return
